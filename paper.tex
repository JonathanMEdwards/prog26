...
\begin{abstract}
  ...
  We evaluate Baseline with case studies of schema evolution problems identified in a recent paper and a literature review.
...
Grounding: In prior work with others we identified challenge problems of schema evolution in interactive programming systems based on our own experiences and a literature review. We use these problems as case studies to evaluate the usefulness and limitations of Baseline.
...
We evaluate the usefulness of Baseline with case studies of schema evolution scenarios that we (along with other collaborators) identified in an earlier paper, based on our own experiences and a literature review: \textit{Schema Evolution in Interactive Programming Systems}~\cite{challenge-problems}. We explore variations on these cases, prefixed by {\scriptsize\faPlus}, to elucidate the current capabilities and limitations of Baseline.
...
\caseplus
This challenge problem was extracted from \citeauthor*{Cambria}~\cite[Appendex III]{Cambria}, which extends the problem: what happens when \quotedstring{Jack} is deleted from the list? They want to set the old version's single value to a JavaScript \textsf{null}. Baseline currently handles this situation by refusing to retract the \textsf{delete} backwards through the \textsf{ListOf}, decoupling subsequent changes to the old single value from the new list. Perhaps Baseline should instead map deletion to the empty string and vice-versa (or support proper nullable or optional types analogously). This case study is a good example of how ``doing the right thing'' can depend on the context of use---there is no universal truth.
\end{casestudy}
...
\begin{casestudy}
The preceding example of Acme's database evolution is taken verbatim from the challenge problem \textbf{Extract Entity}~\cite{challenge-problems}. Indeed more than being a case study, this problem was the primary motivating example in the design of Baseline.
...
\caseplus
\citet{herrmann17} study the case of a task management system (similar to our \textsf{TODO} example), where they split an \textsf{author} column into a separate table while assigning unique IDs that are used as a foreign key. Their \textsf{Decompose} operation combines our \textsf{Split} operation with deduplication.
...
If only there was a way to do operational diffs that respect provenance.

\section{Evaluation \& Discussion}\label{discussion}

\textcolor{red}{Summarize case studies?}
...
Appendix~\ref{tech-dims} contains an evaluation of Baseline using the \textit{Technical Dimensions of Programming Systems}~\cite{techdims} framework.
\textcolor{red}{absorb the appendix inline here? Conclusions?}

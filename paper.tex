...
We implement version control by comparing and creating timelines. These operations are built upon two primitive functions on timelines: \textsf{Project} and \textsf{Retract}.

\subsection{Projection}
The \textsf{Project} function takes as inputs two timelines with the same initial state and produces two new timelines continuing from the input timelines and converging on the same final state. It is easier to understand this with the diagrams in Figure~\ref{fig:projection}.
...
\begin{tikzcd}
	...
	\arrow["{\textsf{P}\Rightarrow}"{description, pos=0.1}, draw=none, from=1-1, to=2-2]
...
asymmetric: pre overrides base
preserving intention "doing the same thing"

projection rules abstracted from examples
design tradeoffs between actions and rules. design of actions and rules coupled not arbitrary. example?

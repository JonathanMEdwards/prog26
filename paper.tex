...
As noted previously, implementing projection and retraction rules with if statements has proven impractical. We plan to build a DSL for specifying rules more succinctly, exploiting their inherent symmetries.

We need further work to settle on the best set of operations for schema evolution in nested relations. Because of the deduping anomalies mentioned earlier we want to implement a set datatype that automatically collapses duplicates into equivalence classes.

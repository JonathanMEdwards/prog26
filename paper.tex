...
The playable demo is an experimental method of evaluating early-stage research in programming experiences. The question is whether it is more effective on some measures than written narratives and recorded demos? Our early results are mixed.
...
\begin{itemize}
  ...
  \item The data model presented here is a subset of our implementation which is in turn a subset of our goal to integrate the data models of PLs and DBs. Notably missing are sum types, type references, and set-valued deep links with multiplicity/integrity constraints.
  ...
  \item It has been surprisingly difficult to design a complete set of primitive operations supporting relationships across nested tables. Does this point to a broken symmetry?

\item We have been avoiding operations that duplicate information in an attempt to see how far we can get without them. They are a well-known problem for view updates and lenses. The solution may complicate diffing and weaken the transfer roundtrip law.

% \item Type mismatch errors should be encapsulated into \textit{exceptional values} that do not contaminate their environment but are still visible and repairable in the GUI.

\item We need operations for editing formulas/timelines and they need to be surfaced conveniently in the GUI.

\item We need an executeable specification language for project/retract rules.

\item Interoperability is a practical necessity: we need incremental import/export with standard data file formats and APIs.
...
Preliminary work on an operationalized query language is encouraging. We have developed  new UI visualizations and are experimenting with a playable demo as an evaluation method.

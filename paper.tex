...
\item Using these primitives we reconstruct the key features of version control: \textit{diffing} and \textit{transfer} (a specialization of merging more like cherry-picking). Because our technique is operation-based we are able to do high-fidelity diffing and merging across refactorings and structural transformations. For example renaming does not conflict with other changes, and movement of a structure does not conflict with changes inside it.
...
We evaluate the usefulness of Baseline with case studies of schema evolution scenarios taken from the research literature. These include challenge problems we identified in \textit{Schema Evolution in Interactive Programming Systems}~\cite{challenge-problems} that originally motivated Baseline.

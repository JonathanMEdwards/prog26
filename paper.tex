...
\begin{array}{l|l}
  ...
  p \textsf{ moveFrom } p' & \textrm{List element at $p$ is overwritten from $p'$ in same list}\\
  & \textrm{or cousin in nested lists, and $p'$ is deleted}\\
  \hline

  \textrm{Type actions} & \textrm{(Iterated over all values paths matching type path $p$)}
  \rule[0pt]{0pt}{16pt}\\
  \hline
  ...
  p \textsf{ MoveFrom } p' & \textrm{Record field at $p$ is overwritten from $p'$ in same record}\\
  & \textrm{or containing/contained record, and $p'$ is deleted}\\

  p \textsf{ ListOf } & \textrm{Convert value at $p$ into a singleton list with element ID $\mathbb{1}$}\\
  p \textsf{ FirstOf } & \textrm{Convert list at $p$ into its first element, else the initial value}\\

...
\begin{abstract}
  ...
  We discuss the design of Baseline's direct manipuation GUI supporting interactive schema changes as well as diffing and merging between databases. A playable demo is available.
...
The GUI was iterated through three versions by feedback from demonstrations. We report initial experience with a \textit{Playable Demo} as a method of evaluation.
...
%   \item Baseline: Data Evolution and Versioning
%   \item Baseline: Operation-Based Data Evolution and Versioning
%   \item Baseline: Integrating data change across time, space, and meaning
%   \item Baseline: Differentiating data across time, space, and meaning
%   \item Baseline: Operational Differencing for Data Evolution and Versioning
%   \item Baseline: Operation-Based Evolution and Versioning of Data
%   \end{itemize}
%   }

%   Method-focused:
% Operational Differencing: An Operation-Based Approach to Data Evolution and Version Control
% Beyond State: Operational Differencing for Schema Evolution and Rich Data Change Management
% System/Project-focused:
% Baseline: Fluently Managing Data Evolution with Operational Differencing
% Baseline: A General-Purpose Framework for Schema Evolution and Operation-Based Version Control
% Vision-focused:
% Fluent Data Evolution Across Time, Space, and Meaning with Operational Differencing
% Toward Unified Mechanisms for Data Change: Schema Evolution and Version Control in Baseline
...
It is difficult to convey the experience of an interactive user interface in a written narrative, so we have developed a \textit{Playable Demo}. We adopt \textit{onboarding} techniques that commercial software uses to teach new users. A scaffold over the UI walks the user through a demo scenario, stepping through a written narrative that prompts each user action. Users that follow the main quest are given a frustration-free experience going at their own pace. The more adventurous are free to roam at their own risk, but can always respawn where they left the path. The demo can be accessed at \url{http://thebaseline.dev/Programming26/} and we encourage reviewers to share their experience in the survey at the end.
...
\subsection{The Playable Demo}\label{demo}

The playable demo is an experiment in scholarly communication: is it more effective on some measures in conveying early-stage Programming Experience (PX) research results than written narratives and recorded demos? Our early results are mixed.
...
% collapse into prose to save space?
\begin{itemize}

  \item We currently use \textsf{move} operations to dedup, because that was the simplest thing that could possibly work, but it turns out it doesn't. The data model needs to directly support deduped set-like tables.

  \item It has been surprisingly difficult to design a complete set of primitive operations supporting relationships across nested tables. This needs to be settled.

\item We have been avoiding operations that duplicate information in an attempt to see how far we can get without them. They are a known problem for view updates and lenses. The solution may complicate diffing and weaken the transfer law.

\item We need operations for editing formulas/timelines and they need to be surfaced conveniently in the GUI.

\item We need an executeable specification language for transfer rules.

\item Interoperability is a practical necessity. We need incremental import/export to standard data file formats and APIs.

\item Two branches might evolve in parallel to the same state, but assign different IDs in insert operations. This is one situation were state-based approaches have an advantage. Diffing should detect such situations and allow the divergence to be collapsed (with \textsf{move} operations).

\item Should batch transfers support some form of transactional consistency?

\item The next major phase of this research is to explore the conjecture that queries can be operationalized. Can that be extended all the way into a full programming language? If so it would open up an unexplored space of language designs.

\end{itemize}

\noindent\rule{\textwidth}{1pt}
To summarize,
Operational Differencing provides a new kind of version control on data with capabilities beyond known techniques. It would be fair to say that the current state of this research raises more questions than it answers, but we hope that is taken as a positive sign of progress and potential.

% vision
% VC for everything, all the way down and up, Lisp redux
% full unified QL/PL with VC semantics, test, supply chain, deployment

%  If we are allowed to speculate, could we get the benefits of version control even at runtime? Could a replay debugger~\cite{Sundmark02, ocallahan16} let us step back in time, hot-patch a change, then roll foward and see the diffs? Could we explore and merge alternative runtime states~\cite{warth11}?
...
% vision: subtrate like Lisp/Smalltalk reengineered for the modern world.

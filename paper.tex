...
Operational Differencing can ``cherry pick'' and merge changes without conflict across substantial schema changes.

The second, unavoidable, conclusion is that Baseline's operations and transfer rules are worryingly complex and incomplete. We are frankly disappointed that an elegant algebra like that of relational databases has not emerged yet. Nevertheless we have a strong intuition that there is a simpler more elegant theory struggling to get out. We report on our interim results here in the hope of sparking discussions that reveal new angles of attack.

On a more technical level, implementing transfer rules in procedural code has proven to be
laborious. We want to define a logic programming DSL for specifying these rules more succinctly that exploits their inherent symmetries.

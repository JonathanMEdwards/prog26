...
%DIF DEL originalsubmission.tex   Sat Feb  7 15:15:59 2026
%DIF ADD paper.tex                Sun Feb 22 13:21:23 2026
...
%DIF >  Calling this section Evaluation makes the latexdiff better
%DIF >  If only there was a more accurate way to do diffing :(
\section{Evaluation}\DIFaddbegin \label{discussion}
\DIFaddend 
...
\cite{challenge-problems}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{\textcolor{red}{Need to add variations to the 3 Extract Entity problems}
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{enumerate}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{\textcolor{red}{Summarize case study results here?}
}\DIFaddend 
...
}\DIFdelend \DIFaddbegin \DIFadd{\textcolor{red}{Discuss 4 failed challenge problems as case studies here?}
%DIF >    \item \textbf{Live State Type Evolution}\redCross The problem is to evolve the state of an Elm application so that it needn't be restarted.
...
\DIFdel{operation taking a regular expression that splits a string column of a table. But we judge that regular expressions violate Baseline's principle that operationsare concrete direct manipulations, so we disclaim a solution.
...
%DIFDELCMD < \greenCheck %%%
\DIFdel{This problem is about allowing different versions of code to interoperate bidirectionally even though they have different schema, specifically a schema evolution converting a scalar valueinto a list. We adopted this problem as the example shown in Figure~\ref{fig:TODO-refactor} and solved it in \S\ref{transfer}. An extended version of this problem is posed by }%DIFDELCMD < \citeauthor*{Cambria}%%%
\DIFdel{~\mbox{%DIFAUXCMD
\cite[Appendex III]{Cambria} }\hskip0pt%DIFAUXCMD
...
%DIFDELCMD < %%%
\DIFdel{We judge that we have solved four of the eight challenge problems. The remaining four are goals for future work.
}\DIFdelend Appendix~\ref{tech-dims} contains an evaluation of Baseline using the \textit{Technical Dimensions of Programming Systems}~\cite{techdims} framework.

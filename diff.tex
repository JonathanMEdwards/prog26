...
%DIF ADD paper.tex                Sun Feb  8 12:31:12 2026
...
\cite{challenge-problems}}\hskip0pt%DIFAUXCMD
. In fact more than a case study, this problem was the primary motivating example in the design of Baseline.
}\end{casestudy}
...
\cite{challenge-problems}}\hskip0pt%DIFAUXCMD
. Our literature review in that paper found surprisingly little prior work on the problems of deduplication and ID assignment despite that being a commonplace situation in database schema evolution. Indeed Baseline's solution is not entirely satisfactory, as discussed in the Note above. Our work on Baseline has taught us that the concept of identity is more problematic than is commonly acknowledged.
}\end{casestudy}
...
\cite{challenge-problems}}\hskip0pt%DIFAUXCMD
. While admittedly partial it appears novel based on our literature review. Other solutions have repurposed query optimization techniques for query rewriting, whereas in Baseline it falls out of the semantics of operation transfer.
}\end{casestudy}
\DIFaddend 
...
%DIFDELCMD < \redCross %%%
\DIFdel{The problem is to take string data in a table column and parse it into multiple columns. Changes should be transferable bidirectionally between the new document and a variant of the old. We have implemented a solution using a }\textsf{\DIFdel{parse}} %DIFAUXCMD
\DIFdel{operation taking a regular expression that splits a string column of a table.
But we judge that regular expressions violate Baseline's principle that operations are concrete direct manipulations, so we disclaim a solution.
...
%DIFDELCMD < %%%
\DIFdel{We judge that we have solved four of the eight challenge problems. The remaining four are goals for future work. }\DIFdelend Appendix~\ref{tech-dims} contains an evaluation of Baseline using the \textit{Technical Dimensions of Programming Systems}~\cite{techdims} framework.

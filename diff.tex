...
%DIF DEL originalsubmission.tex   Wed Feb 25 14:04:08 2026
%DIF ADD paper.tex                Wed Feb 25 14:05:04 2026
...
%DIFDELCMD < \greenCheck %%%
\DIFdel{This problem builds upon the previous one, asking that a query for pending orders be rewritten during the schema evolution. We discussed this example and solved it in \S\ref{query-rewriting}, albeit in a fragmentary query language}\DIFdelend \DIFaddbegin \DIFadd{To evaluate the capabilities of Baseline we have examined case studies taken from the research literature. These include the challenge problems identified in our prior paper
}\textit{\DIFadd{Schema Evolution in Interactive Programming Systems}}\DIFadd{~\mbox{%DIFAUXCMD
\cite{challenge-problems}}\hskip0pt%DIFAUXCMD
. We have already discussed four of these challenge problems in case studies \ref{case-multiplicity}, \ref{case-extract}, \ref{case-diverge}, and \ref{case-query}}\DIFaddend .
...
%DIFDELCMD < \redCross %%%
\DIFdel{This problem builds upon the previous one, asking that formulas be rewritten during the schema evolution. Baseline does not yet have enough of a formula languageto meet the requirements of this problem.
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdelend %DIF >  Baseline demonstrates novel capabilities but also raises questions requiring further research.

\DIFdelbegin \DIFdel{Baseline demonstrates novel capabilities but also raises questions requiring further research.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend % Baseline assigns a unique ID to every insertion operation. Sometimes that might be undesirable, as when people working in different branches coincidentally make the same changes. Diffing will treat their insertions as different. This is one case where state-based version control has a more intuitive result. The diffing UI should detect such cases of adjacent similar insertions and offer to coalesce them (with \textsf{move} operations).

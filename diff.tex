...
%DIF DEL originalsubmission.tex   Thu Mar  5 10:46:38 2026
%DIF ADD paper.tex                Thu Mar  5 13:50:31 2026
...
\newtheorem*{note}{Note}
%DIF 14a14
\newtheorem{casestudy}{Case study} %DIF > 
%DIF -------
...
\usepackage{hhline}
%DIF 20-21d21
%DIF < \usepackage{circledtext}
%DIF < \usepackage{fontawesome5}
%DIF -------
...
%DIF -------
% Arxiv doesn't like circledtext %DIF > 
%\usepackage{circledtext} %DIF > 
%\circledtextset{resize=real, charf=\LARGE\sffamily} %DIF > 
\usepackage{circledsteps} %DIF > 
\newcommand{\circledtext}[1]{{\sffamily\Circled{#1}}} %DIF > 
%DIF -------

%DIF 26c28
%DIF < % define natbib citet
%DIF -------
% define natbib \citet %DIF > 
%DIF -------
...
%DIF 32a34-38
\newcommand{\figbox}[1]{\tcbox{ %DIF > 
  \begin{minipage}{\textwidth} %DIF > 
  \vspace{-14pt} %DIF > 
  #1 %DIF > 
  \end{minipage}}} %DIF > 
%DIF -------
...
%DIF 41a48-55
 %DIF > 
% Tighten up spacing %DIF > 
% \setlength{\textfloatsep}{8pt} %DIF > 
% \setlength{\abovedisplayskip}{4pt} %DIF > 
% \setlength{\belowdisplayskip}{4pt} %DIF > 
\setlength{\belowcaptionskip}{-6pt} %DIF > 
 %DIF > 
\newcommand{\myparagraph}[1]{\vspace{-8pt}\paragraph{#1}} %DIF > 
%DIF -------
...
\begin{abstract}
  ...
  This approach to data version control \DIFdelbegin \DIFdel{does fine-grained }\DIFdelend \DIFaddbegin \DIFadd{offers high-fidelity }\DIFaddend diffing and merging despite intervening structural transformations like schema changes. It offers users a simplified conceptual model of version control for ad hoc usage: There is no repo; Branching is just copying. The \DIFdelbegin \DIFdel{informaton }\DIFdelend \DIFaddbegin \DIFadd{information }\DIFaddend maintained in a repo can be synthesized more precisely from the append-only histories of branches. Branches can be flexibly shared as is commonly done with document files, except with the added benefit of diffing and merging.
...
\DIFdelbegin \DIFdel{Altogether we develop solutions to four of the eight challenge problems of schema evolution }\DIFdelend \DIFaddbegin 

  \DIFadd{We evaluate Baseline with case studies of schema evolution problems identified iin a literature review, including the set of challenge problems we }\DIFaddend identified in a \DIFdelbegin \DIFdel{recent }\DIFdelend \DIFaddbegin \DIFadd{prior }\DIFaddend paper.


%DIF >  Prog Journal rubric for abstract. This is annoying to readers. Remove in camera ready!
\DIFaddbegin 
...
Grounding: \DIFdelbegin \DIFdel{In prior work with others we identified eight challenge problems }\DIFdelend \DIFaddbegin \DIFadd{We evaluate Baseline with case studies }\DIFaddend of schema evolution \DIFdelbegin \DIFdel{in interactive programming systems; We solve four of them}\DIFdelend \DIFaddbegin \DIFadd{problems identified iin a literature review, including the set of challenge problems we identified in a prior paper}\DIFaddend .
...
Baseline is a research project to develop a general purpose mechanism for data change in all these dimensions, and to provide it conveniently to both programs and \DIFdelbegin \DIFdel{humans}\DIFdelend \DIFaddbegin \DIFadd{users}\DIFaddend .
...
\DIFdelbegin \DIFdel{These problems have guided our work and are used as examples in the rest of the paper.
}\DIFdelend %DIF >  These problems have guided our work and are used as examples in the rest of the paper.
...
%DIFDELCMD < \includegraphics[width=\textwidth]{GUI.png}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{The Baseline UI}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:UI-plain}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend %DIF >  \begin{figure}
%DIF >  \includegraphics[width=\textwidth]{GUI.png}
%DIF >  \caption{The Baseline UI}
%DIF >  \label{fig:UI-plain}
%DIF >  \end{figure}

Baseline manifests all operations in a direction manipulation UI \DIFdelbegin \DIFdel{(Figure~\ref{fig:UI-plain}) }\DIFdelend that gives users the same power as programs.
It is difficult to convey the experience of an interactive interface in a written narrative, so we offer the reader a playable demo at \url{https://thebaseline.dev/Prog26submission/}.
\DIFdelbegin \DIFdel{The UI and playable demo are discussed in }\DIFdelend \DIFaddbegin \DIFadd{Space limitations force us to move discussion of the UI and the demo into }\DIFaddend Appendices \ref{UI} and \ref{demo}.
...
including refactorings. We define the primitive transformations \textit{projection} and \textit{retraction} \DIFdelbegin \DIFdel{and the rules governing them}\DIFdelend \DIFaddbegin \DIFadd{with rules expressed as commutative diagrams}\DIFaddend .

\item Using these primitives we \DIFdelbegin \DIFdel{reconstruct the key features }\DIFdelend \DIFaddbegin \DIFadd{construct the key capabilities }\DIFaddend of version control: \textit{diffing} and \textit{transfer} (a \DIFdelbegin \DIFdel{specialization of merging more like cherry-picking). Because our technique is }\DIFdelend \DIFaddbegin \DIFadd{merging primitive somewhat like git's cherry-pick). Being }\DIFaddend operation-based \DIFdelbegin \DIFdel{we are able to do fine-grained }\DIFdelend \DIFaddbegin \DIFadd{provides high-fidelity }\DIFaddend diffing and merging \DIFdelbegin \DIFdel{despite intervening }\DIFdelend \DIFaddbegin \DIFadd{across refactorings and }\DIFaddend structural transformations. \DIFaddbegin \DIFadd{For example renaming doesn't conflict with other changes, and movement of a structure doesn't conflict with changes within it.
}\DIFaddend 

\item We \DIFdelbegin \DIFdel{develop a simplified form }\DIFdelend \DIFaddbegin \DIFadd{propose a simplified model }\DIFaddend of version control without a repo: \DIFdelbegin \DIFdel{artifacts under version control }\DIFdelend \DIFaddbegin \DIFadd{documents }\DIFaddend have an append-only history of operations\DIFdelbegin \DIFdel{attached to them. Branching }\DIFdelend \DIFaddbegin \DIFadd{; branching }\DIFaddend is just making a copy of the artifact with its history, which can then diverge. \DIFdelbegin \DIFdel{There is no need for a repo to record a }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend graph of branches and merges \DIFdelbegin \DIFdel{because that }\DIFdelend \DIFaddbegin \DIFadd{does not need to recorded in a repo because it }\DIFaddend can be synthesized from the histories on need, and can be done so optimally in a \DIFdelbegin \DIFdel{well-defined sense . By liberating branches and versions of artifacts }\DIFdelend \DIFaddbegin \DIFadd{sense we define. Liberating versions and branches }\DIFaddend from the repo \DIFdelbegin \DIFdel{they can be shared freely and independently, much like }\DIFdelend \DIFaddbegin \DIFadd{frees them to be shared independently, as in }\DIFaddend the ubiquitous folk practice of \DIFdelbegin \DIFdel{file copying }\DIFdelend \DIFaddbegin \DIFadd{copying spreadsheets}\DIFaddend ~\cite{Burnett14, Basman19}, but with the added benefit of being able to diff and merge.

\item We enrich the data model to incorporate relational databases, including operations for schema change and normalization\DIFdelbegin \DIFdel{. We demonstrate }\DIFdelend \DIFaddbegin \DIFadd{, providing }\DIFaddend version control on databases that works across schema changes \DIFdelbegin \DIFdel{, and which can also be used to manage schema migrations}\DIFdelend \DIFaddbegin \DIFadd{and automates schema migration}\DIFaddend .

\item We conjecture that queries can be \textit{operationalized} into a sequence of schema and data operations, in a form of Programming by Demonstration~\cite{cypher93-pbd}. We develop this idea on a query language fragment \DIFdelbegin \DIFdel{containing }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend selects and joins.
\DIFdelbegin \DIFdel{Operationalized queries are represented as a }\textit{\DIFdel{future timeline}} %DIFAUXCMD
\DIFdel{that is speculatively executed as a branch off of the present state, returning a value from its }\textit{\DIFdel{hypothetical future}}%DIFAUXCMD
\DIFdel{. This conjecture could open up an unexplored space of query language designs.
}%DIFDELCMD < 

%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Operationalized queries have a perhaps surprising benefit: query }\DIFdelend %DIF >  Operationalized queries are represented as a \textit{future timeline} that is speculatively executed as a branch off of the present state, returning a value from its \textit{hypothetical future}. This conjecture could open up an unexplored space of query language designs.
\DIFaddbegin \DIFadd{Query }\DIFaddend rewriting~\cite{curino08, herrmann17} during schema change falls out ``for free''\DIFdelbegin \DIFdel{from the machinery of operational differencing}\DIFdelend .
...
\item%DIFAUXCMD
\DIFdel{Altogether we develop solutions to four out of eight challenge problems }\DIFdelend %DIF >  \item Altogether we develop solutions to four out of eight challenge problems of schema evolution~\cite{challenge-problems}.
\DIFaddbegin 

\end{enumerate}

\DIFadd{We evaluate the capabilities and usefulness of Baseline with case studies }\DIFaddend of schema evolution \DIFaddbegin \DIFadd{scenarios taken from the research literature. These include the challenge problems we identified earlier}\DIFaddend ~\cite{challenge-problems}.

\DIFdelbegin %DIFDELCMD < \end{enumerate}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend % The superpower of our operation-based approach to schema evolution and version control is that, compared to traditional state-based approaches, it captures user intentions more accurately. We also seek to make these capabilities more concrete and interactive through a direct manipulation UI, reducing as much as we can the need for ad hoc scripting. Throughout, we seek to make things easier though conceptual simplification.
...
% data syntax table
\begin{figure}[h]
  \DIFdelbeginFL %DIFDELCMD < \tcbox{
%DIFDELCMD <   \begin{minipage}{\textwidth}
%DIFDELCMD < \[ \begin{array}{r@{\ }l|r@{\ }l|r@{\ }l|l}
%DIFDELCMD <   \multicolumn{2}{l|}{\textrm{type}} & \multicolumn{2}{l|}{\textrm{value}} & \multicolumn{2}{l|}{\textrm{initial value}}&\\
%DIFDELCMD <   \hline
%DIFDELCMD <   T \Coloneqq & & v \Coloneqq & & T^\varnothing = & &\\
%DIFDELCMD <   &  \textsf{String} & & S & & \emptystring & \textrm{string}\\
%DIFDELCMD <   & \textsf{Number} & &  N & & \textsf{NaN} & \textrm{number}\\
%DIFDELCMD <   & \textsf{List } T & & [ E \is v \  \dots ] & & [] & \textrm{list}\\
%DIFDELCMD <   %& \{ F \  S \is T \  \dots \} & & \{ F \is v \  \dots \} & & \{ F \is T^\varnothing \  \dots \}& \textrm{record}\\
%DIFDELCMD <   & \{ F \  S \is T \  \dots \} & & \{ F \is v \  \dots \} & & \{ F \is T^\varnothing \  \dots \}& \textrm{record}\\
%DIFDELCMD < 

%DIFDELCMD <   & \bot && \bigtimes & & \bigtimes & \textrm{tombstone}\\
%DIFDELCMD < \end{array}\]
%DIFDELCMD < 

%DIFDELCMD < % data operations table
%DIFDELCMD < \[
%DIFDELCMD < \hspace{-22pt}
%DIFDELCMD < \begin{array}{l|l}
%DIFDELCMD <   \textrm{Value operations} & \\
%DIFDELCMD <   \hline
%DIFDELCMD <   p \textsf{ noop} & \textrm{Do nothing}\\
%DIFDELCMD <   p \textsf{ write } v & \textrm{Write atomic value $v$ at $p$}\\
%DIFDELCMD <   % p \textsf{ init } & \textrm{Set $p$ to initial value}\\
%DIFDELCMD < 

%DIFDELCMD <   p \textsf{ insert } E \textsf{ before } E' & \textrm{Insert element ID $E$ into list at $p$ in front of element $E'$}\\
%DIFDELCMD <   p \textsf{ append } E & \textrm{Append element ID $E$ to end of list at $p$}\\
%DIFDELCMD < 

%DIFDELCMD <   p \textsf{ delete } E & \textrm{Delete element ID $E$ in list at $p$}\\
%DIFDELCMD < 

%DIFDELCMD <   p \textsf{ move } p' & \textrm{List element $p$ is copied to $p'$ in same list then $p$ is deleted}\\
%DIFDELCMD <   \hline
%DIFDELCMD < 

%DIFDELCMD <   \textrm{Type operations} & \rule[0pt]{0pt}{16pt}\\
%DIFDELCMD <   \hline
%DIFDELCMD <   p \textsf{ Define } T & \textrm{Define $p$ to have type $T$, initializing values}\\
%DIFDELCMD <   p \textsf{ Convert } T & \textrm{Convert atomically-typed $p$ to atomic type $T$}\\
%DIFDELCMD < 

%DIFDELCMD <   p \textsf{ Rename } S & \textrm{Rename record field $p$ to string $S$}\\
%DIFDELCMD < 

%DIFDELCMD <   p \textsf{ Insert } F \textsf{ before } F' & \textrm{Insert field ID $F$ into record at $p$ in front of field $F'$}\\
%DIFDELCMD <   p \textsf{ Append } F & \textrm{Append field ID $F$ to end of record at $p$}\\
%DIFDELCMD < 

%DIFDELCMD <   p \textsf{ Delete } F & \textrm{Delete field ID $F$ in record at $p$}\\
%DIFDELCMD < 

%DIFDELCMD <   p \textsf{ Move } p' & \textrm{Record field at $p$ is copied to $p'$ in the same record}\\
%DIFDELCMD <   & \textrm{or containing\slash contained record, and then $p$ is deleted}\\
%DIFDELCMD < 

%DIFDELCMD <   p \textsf{ ListOf } & \textrm{Convert value at $p$ into a list of one element with ID $\mathbb{1}$}\\
%DIFDELCMD <   p \textsf{ IntoFirst } & \textrm{Convert list at $p$ into its first element, else the initial value} \\
%DIFDELCMD < 

%DIFDELCMD <   p \textsf{ RecordOf } F & \textrm{Convert value at $p$ into a record of one field with ID $F$} \\
%DIFDELCMD <   p \textsf{ IntoField } F & \textrm{Convert record at $p$ into value of its field $F$} \\
%DIFDELCMD < 
...
%DIFDELCMD < }
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \vspace{-8pt}
\figbox{
...
\vspace{-4pt}
\DIFaddendFL \caption{Rich data syntax and operations}
\label{fig:rich-data}
\end{figure}
...
\DIFdelbegin \DIFdel{Operation }\DIFdelend \DIFaddbegin \DIFadd{Operations }\DIFaddend use infix syntax, placing the target path first, then the operation name, followed by any other parameters.
...
The \textsf{append} and \textsf{write} operations are \textit{value operations}, meaning they do not modify the type of the state. On the other hand \textsf{Append} and \textsf{Define} are \textit{type \DIFdelbegin \DIFdel{operatons}\DIFdelend \DIFaddbegin \DIFadd{operations}\DIFaddend } that may modify both the type and value. Type operations do \textit{schema migration}: the value is adapted to match the type while minimizing loss of information, iterating over list elements as needed (think of the \mathbox{*} in the type path as a wildcard). By convention we capitalize the name of type operations. Baseline has a \textit{user mode} in which only value operations are permitted.
...
For example a common design evolution is when a single value needs to become a list of multiple values. We capture this in the \textsf{ListOf} operation, shown on a \DIFdelbegin \DIFdel{TODO table }\DIFdelend \DIFaddbegin \DIFadd{table of tasks for a TODO application }\DIFaddend in Figure~\ref{fig:TODO-refactor}. \DIFaddbegin \DIFadd{We want to change tasks from having a single assigned person to a list of people.
}\DIFaddend The \textsf{ListOf} operation takes any type and wraps it inside a list, and wraps all corresponding values into single-element lists using the element ID $\mathbb{1}$. We will return to this example in the next section.


% \subsection{Much Ado About Nothing}
% Possibly merge with deletion
% Conjecture: don’t need Null, Option, Result
%DIF <  Snobol and verse show we don't need Results
%DIF >  SNOBOL and verse show we don't need Results
...
\DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{On the other hand, the }\DIFaddend benefit of state-based techniques is that they can observe other systems from the outside, which may explain why all existing version control systems are state-based.
...
\DIFaddendFL \begin{minipage}[b][][b]{1.75in}
\center
...
\DIFdelbegin \subsection{\DIFdel{Retraction}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend % retraction diagrams
\begin{figure}[h]
...
\DIFaddbegin \begin{figure}[]
  \vspace{-20pt}
\[
\DIFaddFL{\textsf{Transfer}(a_{1\ldots n}, O, b_{1\ldots m}) = b' \textrm{ in }\quad
}\begin{tikzcd}[column sep = huge]
	\DIFaddFL{A_{n-1} }& \DIFaddFL{O }& \DIFaddFL{B }\\
	\DIFaddFL{A_n }& {\DIFaddFL{O'}} & {\DIFaddFL{B'}}
	\DIFaddFL{\arrow["{a_n}"', from=1-1, to=2-1]
...
\vspace{-16pt}
\caption{\DIFaddFL{Transfer}}
\label{fig:transfer}
\end{figure}


\DIFaddend \subsection{Across the Multiverse}\label{transfer}
...
\DIFdelbegin %DIFDELCMD < \vspace{-16pt}
%DIFDELCMD < \begin{figure}[h]
%DIFDELCMD < %%%
\[
\DIFdelFL{\textsf{Transfer}(a_{1\ldots n}, O, b_{1\ldots m}) = b' \textrm{ in }\quad
}\begin{tikzcd}[column sep = huge]
	\DIFdelFL{A_{n-1} }& \DIFdelFL{O }& \DIFdelFL{B }\\
	\DIFdelFL{A_n }& {\DIFdelFL{O'}} & {\DIFdelFL{B'}}
	\DIFdelFL{\arrow["{a_n}"', from=1-1, to=2-1]
	\arrow["{\textsf{R}\Rightarrow}"{description, pos=0.1}, draw=none, from=1-1, to=2-2]
	\arrow["{a_{n-1}\,\ldots\  a_1}"', from=1-2, to=1-1]
	\arrow["{b_1\,\ldots \  b_m}", from=1-2, to=1-3]
	\arrow["o", dashed, from=1-2, to=2-2]
	...
	\arrow["{\mathit{adjust}_a}"', dashed, from=2-2, to=2-1]
	\arrow["{\mathit{adjust}_b}", dashed, from=2-2, to=2-3]
}\end{tikzcd}\DIFdelFL{}\]%DIFAUXCMD
%DIFDELCMD < \vspace{-16pt}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Transfer}}
%DIFAUXCMD
%DIFDELCMD < \label{fig:transfer}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend 

Transfer
\DIFdelbegin \footnote{\DIFdel{Transfer was called migrate in earlier publications~\mbox{%DIFAUXCMD
\cite{edwards21, edwards22}}\hskip0pt%DIFAUXCMD
.}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend %DIF >  (called \textit{migrate} in earlier work~\cite{edwards21, edwards22})
retracts \mathbox{a_n} backwards through all earlier operations on the \mathbox{A} timeline onto the original state \mathbox{O} and then projects that operation forwards through all the operations of the \mathbox{B} timeline. The final result is the operation \mathbox{b'} that yields a new version \mathbox{B'}. The effect of the transfer is that the final operation \mathbox{b'} preserves the intention of the original operation \mathbox{a_n} given all the operations traversed while traveling backward in time to \mathbox{O} and then forward to \mathbox{B}. Transfer also computes a new diff between \mathbox{A_n} and \mathbox{B'}: the transferred operation is now shared between them in \mathbox{O'} with branching timelines \mathbox{\mathit{adjust}_a} and \mathbox{\mathit{adjust}_b}. This updated diff is used later for diff optimization (\S\ref{diff-synth}).
...
\DIFdelbegin \DIFdel{This }\DIFdelend \DIFaddbegin \DIFadd{The prior }\DIFaddend example demonstrates how transference can propagate changes bidirectionally through structural changes. Standard state-based version control techniques would see only a big conflict and require a human to pick out and propagate finer-grained changes correctly.
...
\DIFaddbegin \begin{casestudy}\label{case-multiplicity}
\DIFadd{The preceding example of a TODO refactoring was taken directly from the challenge problem }\textbf{\DIFadd{Multiplicity Change in Schema}}\DIFadd{~\mbox{%DIFAUXCMD
\cite{challenge-problems}}\hskip0pt%DIFAUXCMD
. The problem is that different versions of the TODO application must interoperate after the schema evolution from a single assignee to a list. Specifically, changes to the single value \quotedstring{Jack} should be mapped into the list element copying it and vice-versa, despite insertions in the list. Baseline solves this problem by giving that list element the special ID $\mathbb{1}$ to persistently identify it.
}\end{casestudy}


\begin{casestudy}
\DIFadd{Case study \ref{case-multiplicity} was extracted from }\citeauthor*{Cambria}\DIFadd{~\mbox{%DIFAUXCMD
\cite[Appendex III]{Cambria}}\hskip0pt%DIFAUXCMD
, which additionally asked: what happens when \quotedstring{Jack} is deleted from the list? They want to set the old version's single value to a JavaScript }\textsf{\DIFadd{null}}\DIFadd{. Baseline currently handles this situation by refusing to retract the }\textsf{\DIFadd{delete}} \DIFadd{backwards through the }\textsf{\DIFadd{ListOf}}\DIFadd{, decoupling subsequent changes to the old single value from the new list. Perhaps Baseline should instead map deletion to the empty string and vice-versa (or support proper nullable or optional types analogously). This case study is a good example of how ``doing the right thing'' can depend on the context of use---there is no universal truth.
}\end{casestudy}


\DIFaddend \subsection{Diff Optimization and Synthesis}\label{diff-synth}
...
\DIFdelbegin \section{\DIFdel{Database Evolution}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{db-evolution}
%DIFDELCMD < %%%
%DIF <  \section{Relationships}
%DIF <  \section{Relational Databases}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \begin{figure}
  \DIFaddbeginFL \vspace{-8pt}
\DIFaddendFL \begin{subfigure}[b]{35em}\vspace{0pt}
  \sffamily
  \small
...
\end{subfigure}
\DIFaddbeginFL \vspace{-8pt}
\DIFaddendFL \caption{Table schema change}
\label{fig:tables}
\end{figure}


%DIF <  database syntax and operations
\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD < \tcbox{
%DIFDELCMD <   \begin{minipage}{\textwidth}
%DIFDELCMD < \[\begin{array}{r@{\ }l|r@{\ }l|r@{\ }l|l}
...
%DIFDELCMD <   & \rightarrow\! p & & \rightarrow\! E & & \rightarrow\! *
%DIFDELCMD <   & \textrm{link to element $E$ of list $p$ ($*$ is null link)}  \\
%DIFDELCMD < 

%DIFDELCMD <   % multi-link
%DIFDELCMD <   % & \rightarrow\! p & & \rightarrow\! [ E \  \dots ] & & \rightarrow\! []
%DIFDELCMD <   % & \textrm{link into list } p \\
%DIFDELCMD < \end{array}\]
...
%DIFDELCMD <   p \textsf{ link } E & \textrm{Link to element } E \textsf{ (unlink if $*$)} \\
%DIFDELCMD < 

%DIFDELCMD <   % multi-link
%DIFDELCMD <   % p \textsf{ link } E & \textrm{Link to element } E \\
%DIFDELCMD <   % p \textsf{ unlink } E & \textrm{Unlink from element } E \\
%DIFDELCMD <   \hline
%DIFDELCMD < 
...
%DIFDELCMD <   p \textsf{ Link } p' & \textrm{Define $p$ as link to list $p'$} \\
%DIFDELCMD < 

%DIFDELCMD <   p \textsf{ Split } p' & \textrm{Split at column $p$ creating linked table $p'$}\\
%DIFDELCMD <   p \textsf{ Join } & \textrm{Join link column $p$ deleteing linked table}\\
%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Additional database syntax and operations}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:db-addons}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \section{\DIFadd{Database Evolution}}\label{db-evolution}
\DIFaddend 
...
\vspace{-4pt}
  \caption{\DIFaddFL{Additional database syntax and operations}}
  \label{fig:db-addons}
\end{figure}



\DIFaddend The first step in evolving the Acme database is to \textsf{Split} out a \textsf{Customers} table with the following operations:
\[
\begin{array}{ll}
...
(4) The \textsf{Rename} operation renames the links column to be \textsf{customer} \DIFdelbegin \footnote{\DIFdel{Abusing notation by conflating the name and ID}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{.
The result is shown }\DIFdelend \DIFaddbegin \DIFadd{(abusing notation by conflating the name and ID) resulting }\DIFaddend in
Figure~\ref{fig:tables-split}.
...
\DIFaddbegin \begin{casestudy}\label{case-extract}
\DIFadd{The preceding example of Acme's database evolution is taken verbatim from the challenge problem }\textbf{\DIFadd{Extract Entity}}\DIFadd{~\mbox{%DIFAUXCMD
\cite{challenge-problems}}\hskip0pt%DIFAUXCMD
. Indeed this problem was a primary motivating example in the design of Baseline.
}\end{casestudy}


\begin{casestudy}
\citet{sculpin} \DIFadd{study restructuring JSON data. Their example of art exhibition data would be modelled relationally (unnormalized) as a table of exhibits containing nested tables of locations. They map the exhibit title into each location and then extract a single concatenated list of the enhanced locations. Baseline's data model supports nested structures like JSON. The }\textsf{\DIFadd{Split}} \DIFadd{operation on tables is actually composed from two more general operations on nested structures. The }\textsf{\DIFadd{Extract}} \DIFadd{operation replaces content in possibly nested lists with links to a concatenated list of the contents. The }\textsf{\DIFadd{Push}} \DIFadd{operation maps a column of a table through another column containing links. These more general operations can perform the JSON restructuring example.
}\end{casestudy}
...
%DIFDELCMD < %%%
\DIFdel{Above we }\DIFdelend %DIF >  \begin{note}
%DIF >  Above we used a \textsf{move} operation to deduplicate the two coyotes \mathbox{e_1} and \mathbox{e_3}. We took that approach because it was the simplest thing that could possibly work, but arguably it doesn't. Say that \mathbox{e_1} changed its address in the old database and we transfer that change forward into the deduped table. Our current implementation drops the change, because \mathbox{e_3} overwrites \mathbox{e_1}. We believe a more intuitive result would be to ``de-deduplicate''
%DIF >  leaving two coyotes with the same name but different addresses.
%DIF >  We plan to implement a proper set datatype that retains provenance by collapsing duplicates into equivalence classes. This datatype should also simplify the primitive operations for schema evolution in nested relations.
%DIF >  % However it may require weakening the roundtrip law and requiring that the diffing algorithm impose a causal ordering on transferred operations.
%DIF >  % The moral of this story is that it is surprisingly hard to get the transfer semantics of operations right.
%DIF >  \end{note}
\DIFaddbegin 


\begin{casestudy}\label{case-diverge}
\DIFadd{The preceding example is taken from the challenge problem }\textbf{\DIFadd{Divergence Control for Extract Entity}}\DIFadd{~\mbox{%DIFAUXCMD
\cite{challenge-problems}}\hskip0pt%DIFAUXCMD
. Our literature review in that paper found surprisingly little prior work on the problems of deduplication and ID assignment despite that being a commonplace situation in database schema evolution.
It seems managing identity in databases is usually left to the programmer.
Indeed Baseline's solution is not entirely satisfactory either.
We }\DIFaddend used a \textsf{move} operation to deduplicate the two coyotes \mathbox{e_1} and \mathbox{e_3}. We took that approach because it was the simplest thing that could possibly work, but arguably it doesn't. Say that \mathbox{e_1} changed its address in the old database and we transfer that change forward into the deduped table. Our current implementation drops the change, because \mathbox{e_3} overwrites \mathbox{e_1}. We believe a more intuitive result would be to ``de-deduplicate'' leaving two coyotes with the same name but different addresses.
\DIFdelbegin \DIFdel{In \S\ref{discussion} we propose a set-like datatype to solve this problem.
The moral of this story is that it is surprisingly hard to get the transfer semantics of operations right.
}%DIFDELCMD < \end{note}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{We plan to do that by offering a proper }\textsf{\DIFadd{Set}} \DIFadd{datatype that collapses duplicates into equivalence classes of IDs, retaining ID provenance to make change transfers like the above meet intuitive expectations.
%DIF >  This datatype should also simplify the primitive operations for schema evolution in nested relations.
%DIF >  However that may require weakening the roundtrip law and requiring that the diffing algorithm impose a causal ordering on transferred operations.
%DIF >  The moral of this story is that it is surprisingly hard to get the transfer semantics of operations right.
}\end{casestudy}
\DIFaddend 


\DIFaddbegin \begin{casestudy}
\citet{herrmann17} \DIFadd{study the case of a task management system where they split an }\textsf{\DIFadd{author}} \DIFadd{column into a separate table while assigning unique IDs that are used as a foreign key. Their }\textsf{\DIFadd{Decompose}} \DIFadd{operation combines our }\textsf{\DIFadd{Split}} \DIFadd{operation with deduplication.
Their system is not subject to the deduplication anomalies we discussed above, but at the cost of not supporting divergence of data: multiple schema are maintained as updatable views of a single materialized view.
}\end{casestudy}


\DIFaddend \section{Operationalized Queries}
...
is the inverse of \textsf{Split}. It replaces a link column with all the columns of the linked table, copying their contents to match the link values\DIFdelbegin \footnote{\DIFdel{With the set-valued links discussed earlier it effectively becomes a relational join operation.}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{.
%DIF >  \footnote{With the set-valued links discussed earlier it effectively becomes a relational join operation.}.
...
%DIFDELCMD <   & \langle \mathit{op}\fatsemi \ldots \ \uparrow\! p \rangle &&
%DIFDELCMD <   % \textrm{return value} && \textrm{return value} &
%DIFDELCMD <   && &
%DIFDELCMD <   \textrm{Formula returning value at $p$.} \\
%DIFDELCMD < 
...
%DIFDELCMD <   p \textsf{ deletePresent } E & \textrm{Delete rows where column $p$ has non-initial value.} \\
%DIFDELCMD <   p \textsf{ deleteAbsent } E & \textrm{Delete rows where column $p$ has initial value.} \\
%DIFDELCMD < 

%DIFDELCMD <   % p \textsf{ return} & \textrm{Return value at $p$ as result of query.}\\
%DIFDELCMD < 
...
\vspace{-4pt}
  \DIFaddendFL \caption{Additional query syntax and operations}
...
%DIF < \footnote{Operation transfer computes the changes in both directions incrementally which is called \textit{incremental view maintenance}\cite{gupta93}. However performance is not a design goal at this time.}
%DIF >  \footnote{Operation transfer computes the changes in both directions incrementally which is called \textit{incremental view maintenance}\cite{gupta93}. However performance is not a design goal at this time.}


\DIFdelbegin \DIFdel{This selection was a special case: in general we need a language of predicates. We could embed a predicate as a Boolean-valued computed column and then use the }\textsf{\DIFdel{removePresent/removeAbsent}} %DIFAUXCMD
\DIFdel{operations. That would be consistent with the philosophy of }\textit{\DIFdel{querying by sculpting}} %DIFAUXCMD
\DIFdel{but we want to consider adding predicates into the language of operations.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Formulas as Future Timelines}\label{formulas}
...
\DIFdelbegin \subsection{\DIFdel{Query Rewriting}}%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{query-rewriting}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \paragraph{\DIFadd{Query Rewriting}}
\DIFaddend 
...
Forward rewriting is more complicated. It occurs when a formula definition is transferred into a branch containing type differences (i.e., schema changes). Forward rewriting is also done on all formulas within a database whenever a type operation is executed. To transfer a formula forward through a type operation its \textit{inverse} (Appendix~\ref{undo}) is prepended to the formula. The intuition here is that the query was written to apply to the old schema, so to preserve its meaning in the new schema we must first convert the schema back again\DIFdelbegin \footnote{\DIFdel{This naive algorithm would clutter up the formula with irrelevant schema changes. We simplify them out by retracting the original formula back through them as much as possible, retaining only those schema changes the formula is dependent upon.}}%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{.
%DIF >  \footnote{This naive algorithm would clutter up the formula with irrelevant schema changes. We simplify them out by retracting the original formula back through them as much as possible, retaining only those schema changes the formula is dependent upon.}
}\DIFaddend For example if we were to transfer the prior backported query forward again through the \textsf{Split} operation then the inverse operation \textsf{Join} would be prepended to the \textsf{deletePresent} operation, correctly reconstructing the original query.


%DIF <  We offer this as a solution to \textit{Challenge Problem \#3: Code Co-evolution for Extract Entity}~\cite{challenge-problems}.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\section{\DIFdel{Evaluation}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We evaluate Baseline against the challenge problems proposed in
}\textit{\DIFdel{Schema Evolution in Interactive Programming Systems}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{casestudy}\label{case-query}
\DIFadd{This example of query rewriting is a partial solution to the challenge problem }\textbf{\DIFadd{Code Co-evolution for Extract Entity}}\DIFaddend ~\cite{challenge-problems}. \DIFaddbegin \DIFadd{A complete solution would first require a fully general query language. In particular, selection was handled as a special case of testing for blank values; in general a language of predicates is needed.
%DIF >  We could embed a predicate as a Boolean-valued computed column and then use the \textsf{removePresent/removeAbsent} operations. That would be consistent with the philosophy of \textit{querying by sculpting} but we want to consider adding general predicates into the language of operations.
We conjecture that just as queries can be seen as timelines of operations, so can pure functional expressions, as a series of imperative operations on a result value. The hope is that by consistently using timelines as the mechanism of language abstraction the semantics of operational transfer through these timelines would provide refactoring and rewriting ``for free''.
}\end{casestudy}
\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{casestudy}
\citet{curino08} \DIFadd{first introduced }\textit{\DIFadd{Schema Modification Operations}} \DIFadd{using the example of a complex Wikipedia schema evolution. It used selection operations with predicates as discussed above. It also involved making a temporary copy of a table and unioning with another table. Baseline currently does not have either of those operations. They appear straightforward with the caveat that changes to a copy can't be retracted into the original without breaking the roundtrip law.
}\end{casestudy}
\DIFaddend 
...
\DIFdelbegin %DIFDELCMD < \item %%%
\textbf{\DIFdel{Live State Type Evolution}}%DIFAUXCMD
%DIFDELCMD < \redCross %%%
\DIFdel{The problem is to evolve the state of an Elm application so that it needn't be restarted. The example given is to evolve a field of a TODO item from }\textsf{\DIFdel{completed: bool}} %DIFAUXCMD
\DIFdel{to }\textsf{\DIFdel{completed: Maybe<DateTime>}}%DIFAUXCMD
\DIFdel{.
This example raises the question of how Baseline handles optional types and nulls. But it is moot here because Baseline does not yet have enough programming capabilities to implement the TODO application.
}\DIFdelend \DIFaddbegin \section{\DIFadd{Evaluation \& Discussion}}\label{discussion}
\DIFaddend 

\DIFdelbegin %DIFDELCMD < \item %%%
\textbf{\DIFdel{Extract Entity}}%DIFAUXCMD
%DIFDELCMD < \greenCheck %%%
\DIFdel{We adopted this problem verbatim as the example shown in Figure~\ref{fig:tables} and solved in \S\ref{db-evolution} with the }\textsf{\DIFdel{Split}} %DIFAUXCMD
\DIFdel{and }\textsf{\DIFdel{move}} %DIFAUXCMD
\DIFdel{operations. }\DIFdelend \DIFaddbegin \DIFadd{To evaluate the capabilities of Baseline we have examined case studies taken from the research literature. These include the challenge problems identified in our prior paper
}\textit{\DIFadd{Schema Evolution in Interactive Programming Systems}}\DIFadd{~\mbox{%DIFAUXCMD
\cite{challenge-problems}}\hskip0pt%DIFAUXCMD
. We have already discussed four of these challenge problems in case studies \ref{case-multiplicity}, \ref{case-extract}, \ref{case-diverge}, and \ref{case-query}. The other four challenge problems reveal limitations, as follows.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \item %%%
\textbf{\DIFdel{Code Co-evolution for Extract Entity}}%DIFAUXCMD
%DIFDELCMD < \greenCheck %%%
\DIFdel{This problem builds upon the previous one, asking that a query for pending orders be rewritten during the schema evolution. We discussed this example and solved it in \S\ref{query-rewriting}, albeit in a fragmentary query language. }%DIFDELCMD < 

%DIFDELCMD <   \item %%%
\DIFdelend \DIFaddbegin \begin{casestudy}
\DIFadd{The challenge problem }\DIFaddend \textbf{Structured Document Edits}\DIFdelbegin %DIFDELCMD < \redCross %%%
\DIFdel{The problem }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{challenge-problems} }\hskip0pt%DIFAUXCMD
}\DIFaddend is to
take \DIFdelbegin \DIFdel{string data }\DIFdelend \DIFaddbegin \DIFadd{comma-separated strings }\DIFaddend in a table column and \DIFdelbegin \DIFdel{parse it }\DIFdelend \DIFaddbegin \DIFadd{split them }\DIFaddend into multiple columns. Changes should be transferable bidirectionally between the new document and a variant of the old. We have implemented a solution using a \textsf{\DIFdelbegin \DIFdel{parse}\DIFdelend \DIFaddbegin \DIFadd{Match}\DIFaddend } operation \DIFdelbegin \DIFdel{taking }\DIFdelend \DIFaddbegin \DIFadd{that takes }\DIFaddend a regular expression\DIFdelbegin \DIFdel{that splits }\DIFdelend \DIFaddbegin \DIFadd{. This operation takes }\DIFaddend a string column \DIFdelbegin \DIFdel{of a table. But we judge that regular expressions violate }\DIFdelend \DIFaddbegin \DIFadd{and extracts the first match group of the regular expression into another column. While this solves the problem as stated we are not satisfied because the use of regular expressions violates }\DIFaddend Baseline's principle that operations \DIFdelbegin \DIFdel{are }\DIFdelend \DIFaddbegin \DIFadd{correspond to }\DIFaddend concrete direct manipulations \DIFdelbegin \DIFdel{, so we disclaim a solution.
}\DIFdelend \DIFaddbegin \DIFadd{in the UI. We hope that the language of predicates conjectured in case~\ref{case-query} generalizes into a parsing language while still allowing Programming by Demonstration.
}\end{casestudy}
\DIFaddend 

\DIFdelbegin %DIFDELCMD < \item %%%
\textbf{\DIFdel{Code Co-Evolution for Structured Document Edits}}%DIFAUXCMD
%DIFDELCMD < \redCross %%%
\DIFdel{This problem builds upon the previous one, asking that formulas be rewritten during the schema evolution. Baseline does not yet have enough of a formula language to meet the requirements of this problem.
}%DIFDELCMD < 

%DIFDELCMD <   \item %%%
\DIFdelend \DIFaddbegin \begin{casestudy}
\DIFadd{The challenge problems }\DIFaddend \textbf{\DIFdelbegin \DIFdel{Divergence Control for Extract Entity}%DIFDELCMD < \MBLOCKRIGHTBRACE\greenCheck %%%
\DIFdel{This problem extends problem \#2, requiring that changes be bidirectionally transferable through the schema evolution. We solved it in \S\ref{db-vc}, albeit with the well-known limitations of view updates.
}%DIFDELCMD < 

%DIFDELCMD <   \item %%%
\textbf{\DIFdel{Multiplicity Change in Schema}}%DIFAUXCMD
%DIFDELCMD < \greenCheck %%%
\DIFdel{This problem is about allowing different versions of code to interoperate bidirectionally even though they have different schema, specifically a schema evolution converting a scalar value into a list.
We adopted this problem as the example shown in Figure~\ref{fig:TODO-refactor} and solved it in \S\ref{transfer}. An extended version of this problem is posed by }%DIFDELCMD < \citeauthor*{Cambria}%%%
...
%DIFDELCMD <   \item %%%
\DIFdelend \DIFaddbegin \DIFadd{Live State Type Evolution}}\DIFadd{, }\textbf{\DIFadd{Code Co-Evolution for Structured Document Edits}} \DIFadd{and
}\DIFaddend \textbf{Live Editing Domain-Specific Languages}\DIFdelbegin %DIFDELCMD < \redCross %%%
\DIFdel{This problem is the most challenging.
It asks to evolve data and correspondingly adapt the internal execution state of code concurrently operating on it. This appears intractable in general, but the problem is framed in a state-machine DSL where it may be possible.
Baseline lacks enough programming capability to handle this problem yet.
}\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\cite{challenge-problems} }\hskip0pt%DIFAUXCMD
all require programming language capabilities not yet supported in Baseline.
}\end{casestudy}
\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdel{We judge that we have solved four of the eight challenge problems.
The remaining four are goals for future work. Appendix~\ref{tech-dims} contains an evaluation of Baseline using the }\DIFdelend \DIFaddbegin \DIFadd{Baseline is foremost an interactive programming system.
We evaluate and situate its design in Appendix~\ref{tech-dims} using the }\DIFaddend \textit{Technical Dimensions of Programming Systems}~\cite{techdims} framework.
\DIFaddbegin \DIFadd{Much of the work on Baseline has been on its user interface, and especially how to visualize structural differences. Unfortunately space limitations have prevented us from discussing this aspect, which is summarized in Appendix~\ref{UI}.
Ultimately Baseline must be judged in terms of usability.
The playable demo described in Appendix~\ref{demo} has provided some qualitative feedback and could become the basis of a controlled experiment.
%DIF >  We also want to explore the use of the playable demo as a way to communicate UI design ideas.
}\DIFaddend 
...
%DIFDELCMD < %%%
\DIFdel{Baseline demonstrates novel capabilities but also raises questions requiring further research.
}\DIFdelend \DIFaddbegin \DIFadd{There are two main conclusions from our evaluations. First, Baseline offers unique capabilities for version control on databases and documents, the need for which has been documented in field studies~\mbox{%DIFAUXCMD
\cite{Burnett14, Basman19}}\hskip0pt%DIFAUXCMD
.
%DIF >  Working in terms of operations rather than states provides higher-level semantics, as for example the \textsf{Rename} operation that does not conflict with other changes.
Operational Differencing can ``cherry pick'' and merge changes without conflict across substantial schema changes. Doing so by direct manipulation without learning git and without laborious conflict resolution is a substantial benefit to non-technical users.
}\DIFaddend 

\DIFaddbegin \DIFadd{The second, unavoidable, conclusion is that Baseline's operations and transfer rules are worryingly complex and still incomplete. We are frankly disappointed that an elegant algebra like that of relational databases has not emerged yet. Nevertheless we have a strong intuition that there is a simpler more elegant theory struggling to get out. We report on our interim results here in the hope of motivating future research taking new angles of attack. One angle we want to explore is generalizing the data model into a graph to distill a smaller and complete algebra of primitive operations.
}

\DIFadd{Specific technical problems have been discussed where they arose in the case studies, perhaps most notably the need to support a proper set datatype. An overall problem is that implementing transfer rules in procedural code has proven to be unsustainable. We propose to define a logic programming DSL for specifying these rules more succinctly by exploiting their symmetries: }\textsf{\DIFadd{Project}} \DIFadd{and  }\textsf{\DIFadd{Retract}} \DIFadd{are often inverses of each other and their rules are often transposeable.
}

%DIF >  Ideally this DSL can be rendered as diagrams for publication.
%DIF >  This DSL should also support property-based testing of the roundtrip law.

\DIFaddend % Baseline assigns a unique ID to every insertion operation. Sometimes that might be undesirable, as when people working in different branches coincidentally make the same changes. Diffing will treat their insertions as different. This is one case where state-based version control has a more intuitive result. The diffing UI should detect such cases of adjacent similar insertions and offer to coalesce them (with \textsf{move} operations).

\DIFdelbegin \DIFdel{The simple approach of deduplicating by collapsing with }\textsf{\DIFdel{move}} %DIFAUXCMD
\DIFdel{operations leads to transfer anomalies.
We plan to implement a proper set datatype that retains provenance by collapsing duplicates into equivalence classes. This datatype should also simplify the primitive operations for schema evolution in nested relations.
}\DIFdelend %DIF >  The simple approach of deduplicating by collapsing with \textsf{move} operations leads to transfer anomalies. We plan to implement a proper set datatype that retains provenance by collapsing duplicates into equivalence classes. This datatype should also simplify the primitive operations for schema evolution in nested relations.
...
\DIFdelbegin \DIFdel{Implementing projection and retraction rules with procedural if statements is laborious and would benefit from a more declarative approach. It should be possible to define a DSL for specifying rules more succinctly that exploits their inherent symmetries.
%DIF <  Ideally this DSL can be rendered as diagrams for publication.
This DSL should also support property-based testing of the roundtrip law. }\DIFdelend %DIF >  An intriguing question is whether it is possible to define an operationalized query language as we conjecture. Could that be extended into a general purpose functional programming language?

\DIFdelbegin \DIFdel{An intriguing question is whether it is possible to define an operationalized query language as we conjecture. Could that be extended into a general purpose functional programming language?
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend % The data model presented here is a subset of our implementation which is in turn a subset of our goal to integrate the data models of PLs and DBs. Notably missing are sum types, type references (aliases), and set-valued deep links with multiplicity/integrity constraints.
...
%DIFDELCMD < %%%
\DIFdel{Baseline unifies data management across multiple dimensions: mutation over time, collaboration across space, and evolution through design changes. It leverages }\textit{\DIFdel{Operational Differencing}}%DIFAUXCMD
\DIFdel{, a new technique for managing data in terms of high-level operations that include refactorings and schema changes. Integrating schema change operations into a rich data model makes schema evolution a first-class concern instead of an afterthought, enabling solutions to four of the eight challenge problems identified in prior work.
Operational Differencing offers a new kind of version control on data that is finer-grained and higher-fidelity than conventional state-based approaches, able to work across schema evolutions. It also offers a simplified conceptual model for ad hoc usage. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Our preliminary exploration of an operationalized query language opens an intriguing avenue for further research. We believe that Baseline demonstrates, at the least, that operation-based approaches to data management have been under-explored and offer a broad range of new possibilities.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \section{Related Work}

%DIF <  We only have room here to highlight some of the vast adjacent research literature.
\DIFaddbegin \myparagraph{OT and CRDT}
\DIFaddend 

\DIFdelbegin \paragraph{\DIFdel{OT and CRDT}}
%DIFAUXCMD
\addtocounter{paragraph}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend Operational Differencing is related to
...
%DIF <  , and attempts to unify them~\cite{braid}.
Those techniques ensure that multiple replicas distributed across a network will converge to the same state regardless of the order in which operations propagate across the network. To ensure convergence they predetermine how conflicts will be resolved when the operations are first executed. However in version control we need to let humans decide how conflicts will be resolved long after the fact. Version control is about managing divergence not just convergence. On the other hand, most OT and all CRDT algorithms are decentralized, whereas \DIFdelbegin \DIFdel{we are centralized: we can synchronize replicas, but only if one of them is designated as a }\DIFdelend \DIFaddbegin \DIFadd{extending our approach to do replication would require a centralized }\DIFaddend \textit{primary} \DIFdelbegin \DIFdel{serializing all transfers}\DIFdelend \DIFaddbegin \DIFadd{whose order of operations decides conflict resolution for everyone consistently}\DIFaddend .
...
Synql~\cite{ignat-synql} is a CRDT-based approach to replicating relational databases respecting integrity constraints. \DIFaddbegin \DIFadd{Braid~\mbox{%DIFAUXCMD
\cite{braid} }\hskip0pt%DIFAUXCMD
is an attempt to unify OT and CRDT.
}\DIFaddend 

\DIFdelbegin \paragraph{\DIFdel{Databases}}
%DIFAUXCMD
\addtocounter{paragraph}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{The power of CRDTs is their monotonic semantics but that is also their weakness. They can't go backwards, as in OT's ET and our }\textsf{\DIFadd{Retract}}\DIFadd{. Cherry picking a change requires moving it backwards to the most recent ancestor. Going backwards is also essential for undo, especially selective undo (Appendix~\ref{undo}).
}\DIFaddend 

\DIFaddbegin \myparagraph{Databases}

\DIFaddend There is a vast literature on schema evolution in databases, however we will limit the discussion here to the minority of operation-based approaches. A fuller survey can be found in \citet{challenge-problems}.
...
\DIFdelbegin \paragraph{\DIFdel{Data Science}}
%DIFAUXCMD
\addtocounter{paragraph}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \myparagraph{Data Science}
\DIFaddend % The evolution of data formats is a concern in Data Science.
%DIF <  Data~Diff~\cite{sutton18} compares two versions of tabular data and finds a shortest seqeuence of matching transformations.
%DIF >  Data~Diff~\cite{sutton18} compares two versions of tabular data and finds a shortest sequence of matching transformations.
...
\DIFdelbegin \paragraph{\DIFdel{Bidirectional Transformations}}
%DIFAUXCMD
\addtocounter{paragraph}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \myparagraph{Bidirectional Transformations}
\DIFaddend % Programming language theory undergirds research on type-driven transformations.
...
Lenses~\cite{Foster2007,edit-lenses} are a linguistic approach to bidirectional transformations that have been the subject of much research.
 ~\citet{carvalho24} use lenses explicitly specified inside code to express its evolution. \DIFaddbegin \DIFadd{Lenses are an elegant construct but they are essentially first-order: they can transform data changes bidirectionally through schema changes, but they cannot transform schema changes through other schema changes. That is required in order to merge branches whose schema have diverged. A key benefit of Baseline is treating schema and data changes uniformly.
}\DIFaddend 

\DIFdelbegin \paragraph{\DIFdel{Model Driven Engineering}}
%DIFAUXCMD
\addtocounter{paragraph}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \myparagraph{Model Driven Engineering}
\DIFaddend % Model Driven Engineering must also handle evolution.
...
\DIFadd{Baseline unifies data management across multiple dimensions: mutation over time, collaboration across space, and evolution through design changes. It leverages }\textit{\DIFadd{Operational Differencing}}\DIFadd{, a new technique for managing data in terms of high-level operations that include refactorings and schema changes.
Integrating schema change operations into a rich data model makes schema evolution a first-class concern instead of an afterthought.
%DIF >  , enabling solutions to four of the eight challenge problems identified in prior work.
Operational Differencing offers a new kind of version control on data that is finer-grained and higher-fidelity than conventional state-based approaches and is able to work across schema evolutions. It also offers a simplified conceptual model for ad hoc usage.
Our preliminary exploration of an operationalized query language opens an avenue towards incorporating programming language capabilities.
}

\DIFadd{We have identified significant limitations requiring further research, but we
believe that Baseline demonstrates, at least, that operation-based approaches to data management have been under-explored and promise valuable new capabilities.
}



\DIFaddend \acks
%DIF <  We thank Joshua Horowitz, Devamardeep Hayatpur, and the anonymous reviewers for their helpful comments.
\DIFaddbegin \DIFadd{We thank Joshua Horowitz, Devamardeep Hayatpur, and the anonymous reviewers for their helpful comments.
}\DIFaddend 
...
\DIFdelbegin %DIFDELCMD < \circledtextset{resize=real, charf=\LARGE\sffamily}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend The following discussion concentrates on the rationale behind the key UI design decisions.
...
\DIFdelbegin \paragraph{\DIFdel{Interaction}} %DIFAUXCMD
\addtocounter{paragraph}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \myparagraph{Interaction} \DIFaddend Strictly speaking Baseline has two \textbf{modes of interaction}: user and developer, however user mode is a strict subset of developer mode (no type operations) so it is fair to say there is really only one mode of interaction.

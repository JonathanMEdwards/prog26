...
%DIF DEL originalsubmission.tex   Fri Feb 27 14:07:51 2026
%DIF ADD paper.tex                Tue Mar  3 10:48:18 2026
...
\item Using these primitives we reconstruct the key features of version control: \textit{diffing} and \textit{transfer} (a specialization of merging more like cherry-picking). Because our technique is operation-based we are able to do \DIFdelbegin \DIFdel{fine-grained }\DIFdelend \DIFaddbegin \DIFadd{high-fidelity }\DIFaddend diffing and merging \DIFdelbegin \DIFdel{despite intervening }\DIFdelend \DIFaddbegin \DIFadd{across refactorings and }\DIFaddend structural transformations. \DIFaddbegin \DIFadd{For example renaming does not conflict with other changes, and movement of a structure does not conflict with changes inside it.
}\DIFaddend 
...
\DIFadd{We evaluate the usefulness of Baseline with case studies }\DIFaddend of schema evolution \DIFdelbegin \DIFdel{~\mbox{%DIFAUXCMD
\cite{challenge-problems}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{scenarios taken from the research literature. These include challenge problems we identified in }\textit{\DIFadd{Schema Evolution in Interactive Programming Systems}}\DIFadd{~\mbox{%DIFAUXCMD
\cite{challenge-problems} }\hskip0pt%DIFAUXCMD
that originally motivated Baseline}\DIFaddend .
...
%DIF > BEGIN DIFADD
\DIFaddbegin \begin{casestudy}\label{case-query}
\DIFadd{This example of query rewriting is a partial solution to the challenge problem }\textbf{\DIFadd{Code Co-evolution for Extract Entity}}\DIFadd{~\mbox{%DIFAUXCMD
\cite{challenge-problems}}\hskip0pt%DIFAUXCMD
. A complete solution would first require a fully general query language. In particular, selection was handled as a special case of testing for blank values; in general a language of predicates is needed.
...
A typical example of state-based diffing going wrong.
...
\DIFdelend %DIF > BEGIN DIFADD

\DIFdelbegin \DIFdel{We evaluate Baseline against the challenge problems proposed in
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{To evaluate the capabilities of Baseline we have examined case studies taken from the research literature. These include the challenge problems identified in our prior paper
...
%DIFDELCMD < \redCross %%%
\DIFdel{The problem is to evolve the state of an Elm application so that it needn't be restarted.
  The example given is to evolve a field of a TODO item from }\textsf{\DIFdel{completed: bool}} %DIFAUXCMD
\DIFdel{to }\textsf{\DIFdel{completed: Maybe<DateTime>}}%DIFAUXCMD
\DIFdel{. This example raises the question of how Baseline handles optional types and nulls. But it is moot here because Baseline does not yet have enough programming capabilities to implement the TODO application. }%DIFDELCMD < 
...
%DIF <  Baseline assigns a unique ID to every insertion operation. Sometimes that might be undesirable, as when people working in different branches coincidentally make the same changes. Diffing will treat their insertions as different. This is one case where state-based version control has a more intuitive result. The diffing UI should detect such cases of adjacent similar insertions and offer to coalesce them (with \textsf{move} operations).
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The simple approach of deduplicating by collapsing with }\textsf{\DIFdel{move}} %DIFAUXCMD
\DIFdel{operations leads to transfer anomalies. We plan to implement a proper set datatype that retains provenance by collapsing duplicates into equivalence classes. This datatype should also simplify the primitive operations for schema evolution in nested relations.
%DIF <  These operations may require weakening the roundtrip law.
%DIF <   and requiring that the diffing algorithm impose a causal ordering on transferred operations.
}\DIFdelend \DIFaddbegin \DIFadd{The second, unavoidable, conclusion is that Baseline's operations and transfer rules are worryingly complex and still incomplete. We are frankly disappointed that an elegant algebra like that of relational databases has not emerged yet. Nevertheless we have a strong intuition that there is a simpler more elegant theory struggling to get out. We report on our interim results here in the hope of motivating future research taking new angles of attack. One angle we want to explore is generalizing the data model into a graph to distill a smaller and complete algebra of primitive operations.
}\DIFaddend 

\DIFdelbegin \DIFdel{Implementing projection and retraction rules with procedural if statements is laborious and would benefit from a more declarative approach. It should be possible }\DIFdelend \DIFaddbegin \DIFadd{Specific technical problems have been discussed where they arose in the case studies, perhaps most notably the need to support a proper set datatype. An overall problem is that implementing transfer rules in procedural code has proven to be unsustainable. We propose }\DIFaddend to define a \DIFaddbegin \DIFadd{logic programming }\DIFaddend DSL for specifying \DIFaddbegin \DIFadd{these }\DIFaddend rules more succinctly \DIFdelbegin \DIFdel{that exploits their inherent symmetries}\DIFdelend \DIFaddbegin \DIFadd{by exploiting their symmetries: }\textsf{\DIFadd{Project}} \DIFadd{and  }\textsf{\DIFadd{Retract}} \DIFadd{are often inverses of each other and their rules are often transposeable}\DIFaddend .
\DIFaddbegin 

\DIFaddend % Ideally this DSL can be rendered as diagrams for publication.
\DIFdelbegin \DIFdel{This DSL should also support property-based testing of the roundtrip law.
}\DIFdelend %DIF >  This DSL should also support property-based testing of the roundtrip law.
...
}\DIFdelend %DIF >  Baseline assigns a unique ID to every insertion operation. Sometimes that might be undesirable, as when people working in different branches coincidentally make the same changes. Diffing will treat their insertions as different. This is one case where state-based version control has a more intuitive result. The diffing UI should detect such cases of adjacent similar insertions and offer to coalesce them (with \textsf{move} operations).

%DIF >  The simple approach of deduplicating by collapsing with \textsf{move} operations leads to transfer anomalies. We plan to implement a proper set datatype that retains provenance by collapsing duplicates into equivalence classes. This datatype should also simplify the primitive operations for schema evolution in nested relations.
%DIF >  These operations may require weakening the roundtrip law.
%DIF >   and requiring that the diffing algorithm impose a causal ordering on transferred operations.
\DIFaddbegin 

%DIF >  An intriguing question is whether it is possible to define an operationalized query language as we conjecture. Could that be extended into a general purpose functional programming language?

\DIFaddend % The data model presented here is a subset of our implementation which is in turn a subset of our goal to integrate the data models of PLs and DBs. Notably missing are sum types, type references (aliases), and set-valued deep links with multiplicity/integrity constraints.
...
Integrating schema change operations into a rich data model makes schema evolution a first-class concern instead of an afterthought\DIFdelbegin \DIFdel{, enabling solutions to four of the eight challenge problems identified in prior work.
}\DIFdelend \DIFaddbegin \DIFadd{.
%DIF >  , enabling solutions to four of the eight challenge problems identified in prior work.
}\DIFaddend Operational Differencing offers a new kind of version control on data that is finer-grained and higher-fidelity than conventional state-based approaches \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{and is }\DIFaddend able to work across schema evolutions. It also offers a simplified conceptual model for ad hoc usage.

Our preliminary exploration of an operationalized query language opens an intriguing avenue for further research.
We believe that Baseline demonstrates, at the least, that operation-based approaches to data management have been under-explored and offer \DIFdelbegin \DIFdel{a broad range of new possibilities}\DIFdelend \DIFaddbegin \DIFadd{valuable new capabilities}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdelend \paragraph{OT and CRDT}
...
%DIFDELCMD < \circledtextset{resize=real, charf=\LARGE\sffamily}
%DIFDELCMD < %%%
\DIFdelend 

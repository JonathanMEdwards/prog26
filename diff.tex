...
%DIF DEL originalsubmission.tex   Tue Feb 24 13:11:51 2026
%DIF ADD paper.tex                Tue Feb 24 13:11:47 2026
...
% \newcommand{\caseplus}{\noindent {\scriptsize\faPlus} \ } %DIF > 
% \newcommand{\caseplus}{\noindent \textbf{?} \ } %DIF > 
 %DIF > 
% \newcommand{\redCross}{\textcolor{red}{\ \faTimes\ }} %DIF > 
% \newcommand{\greenCheck}{\textcolor{ForestGreen}{\ \faCheck\ }} %DIF > 
 %DIF > 
...
\DIFdelbegin \DIFdel{Altogether we develop solutions to four of the eight challenge problems of schema evolution }\DIFdelend \DIFaddbegin 
  ...
  \DIFadd{We evaluate Baseline with case studies of schema evolution problems }\DIFaddend identified in a recent paper \DIFaddbegin \DIFadd{and a literature review}\DIFaddend .
...
Grounding: In prior work with others we identified \DIFdelbegin \DIFdel{eight }\DIFdelend challenge problems of schema evolution in interactive programming systems \DIFdelbegin \DIFdel{; We solve four of them}\DIFdelend \DIFaddbegin \DIFadd{based on our own experiences and a literature review. We use these problems as case studies to evaluate the usefulness and limitations of Baseline}\DIFaddend .
...
\DIFdelbegin \DIFdel{Align latexdiff. Another example of the limitations of }\DIFdelend \DIFaddbegin \DIFadd{Align latexdiff. Another example of the limitations of }\DIFaddend state-based diffing.
...
\cite{challenge-problems}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{\textcolor{red}{Summarize case studies?}
\textcolor{red}{Talk about failed challenge problems?}
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{enumerate}
%DIFDELCMD < %%%
\DIFdelend %DIF >    \item \textbf{Live State Type Evolution}\redCross The problem is to evolve the state of an Elm application so that it needn't be restarted.
%DIF >    The example given is to evolve a field of a TODO item from \textsf{completed: bool} to \textsf{completed: Maybe<DateTime>}. This example raises the question of how Baseline handles optional types and nulls. But it is moot here because Baseline does not yet have enough programming capabilities to implement the TODO application.
...
\DIFdel{operations. }\DIFdelend %DIF >    \item \textbf{Code Co-Evolution for Structured Document Edits}\redCross This problem builds upon the previous one, asking that formulas be rewritten during the schema evolution. Baseline does not yet have enough of a formula language to meet the requirements of this problem.

\DIFdelbegin %DIFDELCMD < \item %%%
\textbf{\DIFdel{Code Co-evolution for Extract Entity}}%DIFAUXCMD
%DIFDELCMD < \greenCheck %%%
\DIFdel{This problem builds upon the previous one, asking that a query for pending orders be rewritten during the schema evolution. We discussed this example and solved it in \S\ref{query-rewriting}, albeit in a fragmentary query language .
}\DIFdelend %DIF >    \item \textbf{Live Editing Domain-Specific Languages}\redCross This problem is the most challenging. It asks to evolve data and correspondingly adapt the internal execution state of code concurrently operating on it. This appears intractable in general, but the problem is framed in a state-machine DSL where it may be possible. Baseline lacks enough programming capability to handle this problem yet.



\DIFdelbegin %DIFDELCMD < \item %%%
\textbf{\DIFdel{Structured Document Edits}}%DIFAUXCMD
%DIFDELCMD < \redCross %%%
\DIFdel{The problem is to take string data in a table column and parse it into multiple columns.
Changes should be transferable bidirectionally between the new document and a variant of the old. We have implemented a solution using a }\textsf{\DIFdel{parse}} %DIFAUXCMD
\DIFdel{operation taking a regular expression that splits a string column of a table. But we judge that regular expressions violate Baseline's principle that operationsare concrete direct manipulations, so we disclaim a solution. }%DIFDELCMD < 
...
%DIFDELCMD <   \item %%%
\textbf{\DIFdel{Divergence Control for Extract Entity}}%DIFAUXCMD
%DIFDELCMD < \greenCheck %%%
\DIFdel{This problem extends problem \#2, requiring that changes be bidirectionally transferable through the schema evolution. We solved it in \S\ref{db-vc}, albeit with the well-known limitations of view updates. }%DIFDELCMD < 
...
%DIFDELCMD <   \item %%%
\textbf{\DIFdel{Live Editing Domain-Specific Languages}}%DIFAUXCMD
%DIFDELCMD < \redCross %%%
\DIFdel{This problem is the most challenging. It asks to evolve data and correspondingly adapt the internal execution state of code concurrently operating on it.
This appears intractable in general, but the problem is framed in a state-machine DSL where it may be possible. Baseline lacks enough programming capability to handle this problem yet.
}%DIFDELCMD < 
...
The remaining four are goals for future work. }\DIFdelend Appendix~\ref{tech-dims} contains an evaluation of Baseline using the \textit{Technical Dimensions of Programming Systems}~\cite{techdims} framework.
\DIFaddbegin \DIFadd{\textcolor{red}{absorb the appendix inline here? Conclusions?}
}\DIFaddend 

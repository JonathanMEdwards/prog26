...
%DIF DEL originalsubmission.tex   Sun Feb 22 14:38:52 2026
%DIF ADD paper.tex                Sun Feb 22 14:39:20 2026
...
\DIFadd{We evaluate the usefulness of Baseline with case studies }\DIFaddend of schema evolution \DIFaddbegin \DIFadd{scenarios that we (along with other collaborators) identified in an earlier paper, based on our own experiences and a literature review: }\textit{\DIFadd{Schema Evolution in Interactive Programming Systems}}\DIFaddend ~\cite{challenge-problems}. \DIFaddbegin \DIFadd{We explore variations on these cases, prefixed by }{\scriptsize\faPlus}\DIFadd{, to elucidate the current capabilities and limitations of Baseline.
}\DIFaddend 
...
\caseplus
\DIFadd{This challenge problem was extracted from }\citeauthor*{Cambria}\DIFadd{~\mbox{%DIFAUXCMD
\cite[Appendex III]{Cambria}}\hskip0pt%DIFAUXCMD
, which extends the problem: what happens when \quotedstring{Jack} is deleted from the list? They want to set the old version's single value to a JavaScript }\textsf{\DIFadd{null}}\DIFadd{. Baseline currently handles this situation by refusing to retract the }\textsf{\DIFadd{delete}} \DIFadd{backwards through the }\textsf{\DIFadd{ListOf}}\DIFadd{, decoupling subsequent changes to the old single value from the new list. Perhaps Baseline should instead map deletion to the empty string and vice-versa (or support proper nullable or optional types analogously). This case study is a good example of how ``doing the right thing'' can depend on the context of use---there is no universal truth.
}\end{casestudy}
...
\cite{challenge-problems}}\hskip0pt%DIFAUXCMD
. Indeed more than being a case study, this problem was the primary motivating example in the design of Baseline.
}
...
We conjecture that just as queries can be seen as timelines of operations, so can pure functional expressions, as a series of imperative operations on a result value. By consistently using timelines as the mechanism of language abstraction, the semantics of operational transfer through these timelines provides refactoring and rewriting ``for free''.
}\DIFaddend 
...
\DIFdelbegin \DIFdel{If only there was a way to do operational diffs that respect provenance.
}%DIFDELCMD < 
...
%DIFDELCMD < \greenCheck %%%
\DIFdel{This problem builds upon the previous one, asking that a query for pending orders be rewritten during the schema evolution. We discussed this example and solved it in \S\ref{query-rewriting}, albeit in a fragmentary query language }\DIFdelend \DIFaddbegin \DIFadd{If only there was a way to do operational diffs that respect provenance}\DIFaddend .
...
\DIFdel{operation taking a regular expression that splits a string column of a table. But we judge that regular expressions violate Baseline's principle that operationsare concrete direct manipulations, so we disclaim a solution. }\DIFdelend \DIFaddbegin \section{\DIFadd{Evaluation \& Discussion}}\label{discussion}
\DIFaddend 
...
%DIFDELCMD < \greenCheck %%%
\DIFdel{This problem extends problem \#2, requiring that changes be bidirectionally transferable through the schema evolution. We solved it in \S\ref{db-vc}, albeit with the well-known limitations of view updates. }\DIFdelend %DIF >    \item \textbf{Live State Type Evolution}\redCross The problem is to evolve the state of an Elm application so that it needn't be restarted.
...
This appears intractable in general, but the problem is framed in a state-machine DSL where it may be possible.
Baseline lacks enough programming capability to handle this problem yet.
}\DIFdelend %DIF >    \item \textbf{Code Co-Evolution for Structured Document Edits}\redCross This problem builds upon the previous one, asking that formulas be rewritten during the schema evolution. Baseline does not yet have enough of a formula language to meet the requirements of this problem.
...
\DIFdelbegin \DIFdel{We judge that we have solved four of the eight challenge problems. The remaining four are goals for future work. }\DIFdelend Appendix~\ref{tech-dims} contains an evaluation of Baseline using the \textit{Technical Dimensions of Programming Systems}~\cite{techdims} framework.

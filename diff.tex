...
%DIF ADD paper.tex                Sun Feb  8 09:01:47 2026
...
\DIFdelend %DIF >  \section{Evaluation}
...
\cite{challenge-problems}}\hskip0pt%DIFAUXCMD
.
}\DIFdelend %DIF >  We evaluate Baseline against the challenge problems proposed in
%DIF >  \textit{Schema Evolution in Interactive Programming Systems}~\cite{challenge-problems}.

\DIFdelbegin %DIFDELCMD < \begin{enumerate}
%DIFDELCMD < %%%
\DIFdelend %DIF >  \begin{enumerate}
...
\DIFdel{operations.
}\DIFdelend %DIF >    \item \textbf{Extract Entity}\greenCheck We adopted this problem verbatim as the example shown in Figure~\ref{fig:tables} and solved in \S\ref{db-evolution} with the \textsf{Split} and \textsf{move} operations.
...
}\DIFdelend %DIF >    \item \textbf{Code Co-evolution for Extract Entity}\greenCheck This problem builds upon the previous one, asking that a query for pending orders be rewritten during the schema evolution. We discussed this example and solved it in \S\ref{query-rewriting}, albeit in a fragmentary query language.
...
\DIFdel{operation taking a regular expression that splits a string column of a table. But we judge that regular expressions violate Baseline's principle that operations are concrete direct manipulations, so we disclaim a solution.
...
}\DIFdelend %DIF >    \item \textbf{Divergence Control for Extract Entity}\greenCheck This problem extends problem \#2, requiring that changes be bidirectionally transferable through the schema evolution. We solved it in \S\ref{db-vc}, albeit with the well-known limitations of view updates.
...
\DIFdelbegin %DIFDELCMD < \end{enumerate}
%DIFDELCMD < %%%
\DIFdelend %DIF >  \end{enumerate}

\DIFdelbegin \DIFdel{We judge that we have solved four of the eight challenge problems. The remaining four are goals for future work. }\DIFdelend %DIF >  We judge that we have solved four of the eight challenge problems. The remaining four are goals for future work.
\DIFaddbegin 


\section{\DIFadd{Evaluation and Discussion}}\label{discussion}

\DIFadd{\textcolor{red}{Summarize case study results? Discuss failed challenge problems?}
}
...
%DIFDELCMD < %%%
\DIFdelend Baseline demonstrates novel capabilities but also raises questions requiring further research.
